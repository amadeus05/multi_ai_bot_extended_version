...
class OpenInterestFeatureBuilder(FeatureBuilder):
    block_name = "open_interest"
    FEATURE_SPECS = {
        "open_interest_zscore_7d": feature_spec(
            "open_interest_zscore_7d",
            block_name,
            "(open_interest - rolling_mean(open_interest, {OPEN_INTEREST_ZSCORE_WINDOW_1H})) / "
            "rolling_std(open_interest, {OPEN_INTEREST_ZSCORE_WINDOW_1H})",
            description="Open interest z-score on a rolling 7-day window.",
            params=(feature_param("OPEN_INTEREST_ZSCORE_WINDOW_1H", 24 * 7),),
            inputs=("open_interest",),
        ),
        "open_interest_change_pct_8h": feature_spec(
            "open_interest_change_pct_8h",
            block_name,
            "pct_change(open_interest, {OPEN_INTEREST_CHANGE_LOOKBACK_8H})",
            description="Open interest percentage change over the 8-hour lookback.",
            params=(feature_param("OPEN_INTEREST_CHANGE_LOOKBACK_8H", 8),),
            inputs=("open_interest",),
        ),
        "open_interest_change_pct_24h": feature_spec(
            "open_interest_change_pct_24h",
            block_name,
            "pct_change(open_interest, {OPEN_INTEREST_CHANGE_LOOKBACK_24H})",
            description="Open interest percentage change over the 24-hour lookback.",
            params=(feature_param("OPEN_INTEREST_CHANGE_LOOKBACK_24H", 24),),
            inputs=("open_interest",),
        ),
        "open_interest_funding_crowding_1h": feature_spec(
            "open_interest_funding_crowding_1h",
            block_name,
            "open_interest_change_pct_24h * funding_rate_zscore_7d",
            description="Crowding proxy from rising open interest and extreme funding.",
            inputs=("open_interest", "funding_rate"),
            dependencies=("open_interest_change_pct_24h", "funding_rate_zscore_7d"),
        ),
    }
